...
/** Fixed canvas width — layouts are stable regardless of browser window size. */
...
export class CanvasStateManager {
  ...
  /** tldraw IDs of section titles and divider lines — excluded from node collision checks. */
  private sectionChromeIds = new Set<string>();

  constructor(private editor: Editor) {}

  /** Mark a tldraw shape as section chrome (title/divider) so findFreeRect can skip it. */
  addSectionChrome(tldrawId: TLShapeId): void {
    this.sectionChromeIds.add(tldrawId as string);
  }
  ...
  startSection(
    ...
    // Pre-compute all slot positions when a plan is provided (fixed canvas width)
  ...
  computeNodeBounds(
    ...
    // Live computation (no plan, or node not in plan) — uses fixed canvas width
  ...
  getNextSectionY(): number {
    ...
    if (!bounds) return 55;
    return Math.ceil(bounds.maxY) + 120;
  }
  ...
  findFreeRect(
    ...
    padding = 24,
    /** If true, section chrome (titles/dividers) are excluded from collision checks. */
    excludeSectionChrome = false,
  ): { x: number; y: number; w: number; h: number } {
    const MAX_ATTEMPTS = 8;
    let candidate = { ...preferred };

    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      let hits = this.editor.getShapeIdsInsideBounds(
        new Box(
          candidate.x - padding,
          candidate.y - padding,
      ...
      // Exclude section chrome (titles & dividers) so nodes don't collide with them
      if (excludeSectionChrome && this.sectionChromeIds.size > 0) {
        const filtered = new Set<TLShapeId>();
        for (const id of hits) {
          if (!this.sectionChromeIds.has(id as string)) filtered.add(id);
        }
        hits = filtered;
      }
      ...
      if (rightX + candidate.w <= CANVAS_W) {
  ...
  resolveEdge(
    ...
    if (Math.abs(dx) >= Math.abs(dy)) {
      ...
      if (dx >= 0) {
        x1 = Math.round(fromB.x + fromB.w);
        y1 = Math.round(fromCY);
        x2 = Math.round(toB.x);
        y2 = Math.round(toCY);
      } else {
        x1 = Math.round(fromB.x);
        y1 = Math.round(fromCY);
        x2 = Math.round(toB.x + toB.w);
        y2 = Math.round(toCY);
      }
    } else {
      ...
      if (dy >= 0) {
        ...
        y1 = Math.round(fromB.y + fromB.h);
        x2 = Math.round(toCX);
        y2 = Math.round(toB.y);
      } else {
        x1 = Math.round(fromCX);
        y1 = Math.round(fromB.y);
        x2 = Math.round(toCX);
        y2 = Math.round(toB.y + toB.h);
      }
    }
  ...
  resolveNote(
    ...
    switch (pos) {
      case "below":
        return { x: Math.round(b.x), y: Math.round(b.y + b.h + GAP) };
      case "above":
        return { x: Math.round(b.x), y: Math.round(b.y - GAP - 30) };
      case "left":
        return {
          x: Math.round(b.x - 360 - GAP),
          y: Math.round(b.y + b.h / 2 - 15),
        };
      case "right":
        return {
          x: Math.round(b.x + b.w + GAP),
          y: Math.round(b.y + b.h / 2 - 15),
        };
      case "inside":
        return { x: Math.round(b.x + 10), y: Math.round(b.y + 10) };
    }
  }
