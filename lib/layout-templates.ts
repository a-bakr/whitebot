...
const H_GAP = 80; // horizontal clearance edge-to-edge
const V_GAP = 70; // vertical clearance edge-to-edge
const PAD = 60; // canvas left/right padding
const TITLE_H = 110; // space reserved for the section title above content
  ...
  switch (layout) {
    ...
    case "flow-lr": {
      const nodesPerRow = Math.max(
        1,
        Math.floor((usableW + H_GAP) / (sz.w + H_GAP)),
      );
    ...
    case "mindmap": {
      ...
      const cx =
        (center?.x ?? canvasW / 2 - sz.w / 2) + (center?.w ?? sz.w) / 2;
    ...
    case "tree": {
      ...
      // Correctly compute how many nodes are in THIS row (handles partial last rows)
      const firstChildInRow = (row - 1) * COLS + 1;
      const childrenLeft = totalNodes - firstChildInRow;
      const inRow = Math.min(COLS, Math.max(1, childrenLeft));
