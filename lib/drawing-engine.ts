...
function estimateBounds(cmd: DrawCommand): { x: number; y: number; w: number; h: number } | null {
  switch (cmd.cmd) {
    case 'title':      return { x: cmd.x, y: cmd.y, w: 400,            h: 70  }
    case 'text':       return { x: cmd.x, y: cmd.y, w: 350,            h: 40  }
    case 'rect':       return { x: cmd.x, y: cmd.y, w: cmd.w,          h: cmd.h }
    case 'circle':     return { x: cmd.x - cmd.r, y: cmd.y - cmd.r, w: cmd.r * 2, h: cmd.r * 2 }
    case 'bullet':     return { x: cmd.x, y: cmd.y + cmd.index * 40, w: 400, h: 40 }
    case 'highlight':  return { x: cmd.x, y: cmd.y, w: cmd.w,          h: cmd.h }
    case 'circle-em':  return { x: cmd.x, y: cmd.y, w: cmd.w,          h: cmd.h }
    default:           return null
  }
}
...
export class DrawingEngine {
  ...
  private _subscribeToUserInteraction() {
    const update = () => { this._lastUserInteraction = Date.now() }
  ...
  async executeCommand(cmd: DrawCommand) {
    switch (cmd.cmd) {
      ...
      case 'title':        await this.drawTitle(cmd); break
      case 'text':         await this.drawText(cmd);  break
      case 'rect':         this.drawRect(cmd);        break
      case 'circle':       this.drawCircle(cmd);      break
      case 'arrow':        this.drawArrow(cmd);       break
      case 'line':         this.drawLine(cmd);        break
      case 'bullet':       await this.drawBullet(cmd);break
      case 'highlight':    this.drawHighlight(cmd);   break
      case 'underline':    await this.drawUnderline(cmd); break
      case 'circle-em':    await this.drawCircleEm(cmd);  break
      case 'sketch-arrow': this.drawSketchArrow(cmd); break
      // ── Semantic layout commands ──────────────────────────────────────
      case 'section':      await this.drawSection(cmd); break
      case 'node':         this.drawNode(cmd);        break
      case 'edge':         this.drawEdge(cmd);        break
      case 'note':         await this.drawNote(cmd);  break
    }
  }
  ...
  private async animateRichText(id: TLShapeId, fullText: string) {
    ...
    for (const char of fullText) {
      current += char
      this.editor.updateShapes([{ id, type: 'text', props: { richText: toRichText(current) } }])
      await sleep(22)
    }
  }
  ...
  private async drawTitle(cmd: TitleCommand) {
    ...
    this.editor.createShapes([{
      ...
      props: {
        richText: toRichText(''), size: 'xl' as DrawSize,
        color: toTlColor(cmd.color), font: 'draw',
        textAlign: 'middle', autoSize: true,
      },
    }])
  ...
  private async drawText(cmd: TextCommand) {
    ...
    this.editor.createShapes([{
      id, type: 'text', x: pos.x, y: pos.y,
      props: {
        richText: toRichText(''), size: (cmd.size ?? 'm') as DrawSize,
        color: toTlColor(cmd.color), font: 'draw',
        textAlign: 'start', autoSize: true,
      },
    }])
  ...
  private drawRect(cmd: RectCommand) {
    ...
    this.editor.createShapes([{
      ...
      props: {
        geo: 'rectangle', w: cmd.w, h: cmd.h,
        richText: toRichText(cmd.label ?? ''),
        color: toTlColor(cmd.color),
        fill: 'semi', size: 'l', font: 'draw',
        align: 'middle', verticalAlign: 'middle',
      },
    }])
    this.state.register(sid, id, '', { label: cmd.label, color: cmd.color, shape: 'rect' })
  }
  ...
  private drawCircle(cmd: CircleCommand) {
    ...
    this.state.register(sid, id, '', { label: cmd.label, color: cmd.color, shape: 'circle' })
  }
  ...
  private drawArrow(cmd: ArrowCommand) {
    ...
    this.editor.createShapes([{
      id: createShapeId(), type: 'arrow', x: cmd.x1, y: cmd.y1,
      props: {
        start: { x: 0, y: 0 }, end: { x: dx, y: dy },
        richText: toRichText(cmd.label ?? ''),
        color: toTlColor(cmd.color),
        size: 'l', arrowheadEnd: 'arrow', arrowheadStart: 'none', font: 'draw',
      },
    }])
  }
  ...
  private drawLine(cmd: LineCommand) {
    ...
    this.editor.createShapes([{
      ...
      props: {
        start: { x: 0, y: 0 }, end: { x: dx, y: dy },
        color: toTlColor(cmd.color),
        size: 'm', arrowheadEnd: 'none', arrowheadStart: 'none',
      },
    }])
  }
  ...
  private async drawBullet(cmd: BulletCommand) {
    ...
    this.editor.createShapes([{
      id, type: 'text', x: pos.x, y: pos.y,
      props: {
        richText: toRichText(''), size: 'm' as DrawSize,
        color: 'black' as DrawColor, font: 'draw',
        textAlign: 'start', autoSize: true,
      },
    }])
    this.state.register(sid, id, '', { label: cmd.text, shape: 'text' })
  ...
  private drawHighlight(cmd: HighlightCommand) {
    ...
    this.editor.createShapes([{
      id: createShapeId(), type: 'geo', x: cmd.x, y: cmd.y, opacity: 0.35,
      props: {
        geo: 'rectangle', w: cmd.w, h: cmd.h,
  ...
  private async drawUnderline(cmd: UnderlineCommand) {
    ...
    const STEPS = 25
    this.editor.createShapes([{
      id, type: 'arrow', x: cmd.x1, y: cmd.y1,
      props: {
        start: { x: 0, y: 0 }, end: { x: 0, y: 0 },
        color: toTlColor(cmd.color), size: 'l', dash: 'draw',
        arrowheadEnd: 'none', arrowheadStart: 'none',
      },
    }])
    for (let i = 1; i <= STEPS; i++) {
      const t = i / STEPS
      this.editor.updateShapes([{ id, type: 'arrow', props: { end: { x: dx * t, y: dy * t } } }])
      await sleep(16)
    }
  }
  ...
  private async drawCircleEm(cmd: CircleEmCommand) {
    ...
    const STEPS   = 20
    this.editor.createShapes([{
      id, type: 'geo', x: cx, y: cy,
      props: {
        geo: 'ellipse', w: 2, h: 2,
        color: toTlColor(cmd.color), fill: 'none', dash: 'draw', size: 'l',
      },
    }])
    for (let i = 1; i <= STEPS; i++) {
      const t = i / STEPS
      const w = targetW * t, h = targetH * t
      this.editor.updateShapes([{ id, type: 'geo', x: cx - w / 2, y: cy - h / 2, props: { w, h } }])
      await sleep(16)
    }
  }
  ...
  private drawSketchArrow(cmd: SketchArrowCommand) {
    ...
    this.editor.createShapes([{
      ...
      props: {
        ...
        richText: toRichText(cmd.label ?? ''),
        color: toTlColor(cmd.color),
        size: 'l', dash: 'draw', arrowheadEnd: 'arrow', arrowheadStart: 'none', font: 'draw',
      },
    }])
  }
  ...
  private async drawSection(cmd: SectionCommand) {
    ...
    if (sec.yStart > 100) {
      ...
      const vp = this.editor.getViewportPageBounds()
      const PAD = 60
      const xMax = Math.round(vp.w) - PAD
      this.drawLine({ t: 'draw', cmd: 'line', x1: PAD, y1: dividerY, x2: xMax, y2: dividerY, color: 'grey' })
    }

    // Draw the section title
    const vp = this.editor.getViewportPageBounds()
    const titleX = Math.round(vp.w / 2 - 200)
    await this.drawTitle({ t: 'draw', cmd: 'title', id: `${cmd.id}__title`, text: cmd.title, x: titleX, y: sec.yStart })
  }
  ...
  private drawNode(cmd: NodeCommand) {
    ...
    switch (cmd.shape) {
      ...
      case 'diamond': {
        this.editor.createShapes([{
          id, type: 'geo', x: safe.x, y: safe.y,
          props: {
            geo: 'diamond', w: safe.w, h: safe.h,
            richText: toRichText(cmd.label),
            color: toTlColor(cmd.color),
            fill: 'semi', size: 'l', font: 'draw',
            align: 'middle', verticalAlign: 'middle',
      ...
      case 'text': {
        this.editor.createShapes([{
          id, type: 'text', x: safe.x, y: safe.y,
          props: {
            richText: toRichText(cmd.label),
            size: 'm' as DrawSize, color: toTlColor(cmd.color),
            font: 'draw', textAlign: 'start', autoSize: true,
          },
        }])
        break
      }
      default: {
        ...
        this.editor.createShapes([{
          id, type: 'geo', x: safe.x, y: safe.y,
          props: {
            geo: 'rectangle', w: safe.w, h: safe.h,
            richText: toRichText(cmd.label),
            color: toTlColor(cmd.color),
            fill: 'semi', size: 'l', font: 'draw',
            align: 'middle', verticalAlign: 'middle',
  ...
  private drawEdge(cmd: EdgeCommand) {
    ...
    if (!coords) {
      console.warn(`[DrawingEngine] edge: shape "${cmd.from}" or "${cmd.to}" not found yet`)
      return
    }
    ...
    this.editor.createShapes([{
      id: createShapeId(), type: 'arrow', x: x1, y: y1,
      props: {
        start: { x: 0, y: 0 }, end: { x: dx, y: dy },
        richText: toRichText(cmd.label ?? ''),
        color: toTlColor(cmd.color),
